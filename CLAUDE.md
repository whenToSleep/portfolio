...
**Current state (readiness):** Phases 0–5 done; **Phase 6 code done** (branch `phase-6`), Vercel-dashboard hand-off pending (see checklist below). The bilingual EN/UK site is fully CMS-driven on **Next.js 16 + Payload 3** (Neon Postgres), pixel-faithful to the baseline, with **Draft + Publish** in the admin. On-demand revalidation is now wired: data reads are tagged and Payload `afterChange`/`afterDelete` hooks purge the tags, so published edits refresh the SSG pages without a rebuild. Code on GitHub (`whenToSleep/portfolio`, `main` = `dev`); Vercel deploys `main`. Works/images are still **placeholders** (no real illustrations yet).

**Next up:** user runs the Phase 6 **Vercel hand-off** (set `BLOB_READ_WRITE_TOKEN`, confirm env, redeploy, verify a published edit appears live without rebuild). Then **Phase 7** — images/SEO/a11y/perf.
