...
**Current state (readiness):** Phases 0–7 on `main` (merged + pushed). **Pre-phase «Adaptive» code done** (branch `adaptive-mobile`, off `main`; not yet merged) — the site now has a real mobile layout. **Phase 6 Vercel-dashboard hand-off still pending** (Blob token + live verify — checklist below). The bilingual EN/UK site is fully CMS-driven on **Next.js 16 + Payload 3** (Neon Postgres), pixel-faithful to the desktop baseline, with **Draft + Publish** in the admin and on-demand revalidation. Code on GitHub (`whenToSleep/portfolio`, `main` = `dev`); Vercel deploys `main`. Works/images are still **placeholders** (no real illustrations yet).

**Next up:** merge `adaptive-mobile` → `main`; run the Phase 6 **Vercel hand-off** + **Lighthouse** on the preview; then the **content passes** (real copy + illustrations in `/admin`) before **Phase 8** (VPS prod). Open polish carried over: real OG image + favicon, blur placeholders, Home cover image.
