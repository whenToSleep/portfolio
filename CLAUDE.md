...
**Current state (readiness):** Phases 0–7 code done (branches `phase-6`, then `phase-7` off it; not yet merged to `main`). **Phase 6 Vercel-dashboard hand-off still pending** (Blob token + live verify — checklist below). The bilingual EN/UK site is fully CMS-driven on **Next.js 16 + Payload 3** (Neon Postgres), pixel-faithful to the baseline, with **Draft + Publish** in the admin. On-demand revalidation is now wired: data reads are tagged and Payload `afterChange`/`afterDelete` hooks purge the tags, so published edits refresh the SSG pages without a rebuild. Code on GitHub (`whenToSleep/portfolio`, `main` = `dev`); Vercel deploys `main`. Works/images are still **placeholders** (no real illustrations yet).

**Next up:** merge `phase-6`/`phase-7` to `main`; run the Phase 6 **Vercel hand-off** (set `BLOB_READ_WRITE_TOKEN`, confirm env, redeploy, verify a published edit appears live without rebuild) and a **Lighthouse** pass on the preview (target ≥ 90). Then the **content passes** (real copy + real illustrations in `/admin`) before **Phase 8** (VPS prod). Open polish carried over: real OG image + favicon (defaults today), blur placeholders and a Home cover image once real illustrations land.
